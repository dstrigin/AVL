...
namespace AVLTests
{
	...
	public:
		...
		{
			avltree<int> t1;
			Assert::IsTrue(t1.empty());
		}
		TEST_METHOD(CopyAndILConstructor) {
			avltree<int> t1{ 5, 3, 8, 4, 1 };
			avltree<int> t2(t1);
			Assert::IsTrue(t1 == t2);
		}
	};
	...
	public:
		TEST_METHOD(AddTests) {
			...
			t1.insert(1);
			t1.insert(4);
			Assert::IsTrue(t1 == avltree<int>{5, 8, 3, 4, 1});
		}
		TEST_METHOD(FindTest) {
			avltree<int> t1{ 10, 17, 6, 3, 5, 4, 14, 22 };
			Assert::IsTrue(t1.find(14) != t1.end());
			Assert::IsFalse(t1.find(2) != t1.end());
			Assert::IsTrue(t1.find(10) != t1.end());
		}
		TEST_METHOD(UpperBoundTests) {
			avltree<int> t1{ 10, 5, 8, 3, 1, 12, 16, 13 };
			Assert::AreEqual(*t1.upper_bound(1), 3);
			Assert::AreEqual(*t1.upper_bound(2), 3);
			Assert::AreEqual(*t1.upper_bound(3), 5);
			Assert::AreEqual(*t1.upper_bound(5), 8);
			Assert::AreEqual(*t1.upper_bound(8), 10);
			Assert::AreEqual(*t1.upper_bound(9), 10);
			Assert::AreEqual(*t1.upper_bound(10), 12);
			Assert::AreEqual(*t1.upper_bound(12), 13);
			Assert::AreEqual(*t1.upper_bound(13), 16);
			Assert::AreEqual(*t1.upper_bound(14), 16);
		}
		TEST_METHOD(LowerBoundTests) {
			avltree<int> t1{ 10, 5, 8, 3, 1, 12, 16, 13 };
			Assert::AreEqual(*t1.lower_bound(1), 1);
			Assert::AreEqual(*t1.lower_bound(2), 3);
			Assert::AreEqual(*t1.lower_bound(3), 3);
			Assert::AreEqual(*t1.lower_bound(5), 5);
			Assert::AreEqual(*t1.lower_bound(8), 8);
			Assert::AreEqual(*t1.lower_bound(9), 10);
			Assert::AreEqual(*t1.lower_bound(10), 10);
			Assert::AreEqual(*t1.lower_bound(12), 12);
			Assert::AreEqual(*t1.lower_bound(13), 13);
			Assert::AreEqual(*t1.lower_bound(14), 16);
		}
		TEST_METHOD(RemoveTests) {
			avltree<int> t1{ 10, 5, 8, 3, 1, 12, 16, 13 };

			t1.erase(3);
			Assert::IsTrue(t1.content_equal(avltree<int>{10, 5, 8, 1, 12, 16, 13}));

			t1.erase(10);
			Assert::IsTrue(t1.content_equal(avltree<int>{8, 5, 1, 12, 16, 13}));

			t1.erase(16);
			Assert::IsTrue(t1.content_equal(avltree<int>{8, 5, 1, 12, 13}));
		}
	};
	...
	public:
		...
		TEST_METHOD(SetInsertEraseTests)
		{
			...
			Mycont v1(carr, carr + 3);           // abc
			Mycont v2(carr, carr + 3, pred);     // abc
			Mycont v3(carr, carr + 3, pred, al); // abc
			const Mycont v4(carr, carr + 3);     // abc
			v0 = v4;                             // abc
			...
			v0.clear(); // v0 = []
			std::pair<Mycont::iterator, bool> pib = v0.insert('d'); // v0 = [d]
			Assert::IsTrue(*pib.first == 'd' && pib.second);
			Assert::IsTrue(*--v0.end() == 'd');
			// проверка дерева с повторениями на то, что повторения не было
			//pib = v0.insert('d');
			//Assert::IsTrue(*pib.first == 'd' && !pib.second);
			Assert::IsTrue(*std::get<0>(v0.insert('e')) == 'e'); // v0 = [d, e]
			v0.insert(carr, carr + 3); // v0 = [a, b, c, d, e]
			Assert::IsTrue(v0.size() == 5 && *--v0.end() == 'e');
			// тест на контейнер без повторений с повторениями
	...
	public:
		...
		template<typename T> using ContainerTemplate = avltree<T, Mypred, Myal>;
		...
		TEST_METHOD(MultiSetInsEraseTests)
		{
			...
			v0.clear();
			Assert::IsTrue(*std::get<0>(v0.insert('d')) == 'd');
			Assert::IsTrue(*--v0.end() == 'd');
			Assert::IsTrue(*std::get<0>(v0.insert('d')) == 'd', L"Повторная вставка не сработала");
